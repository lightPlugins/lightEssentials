...
public class MorningCommand implements LightCommand {
    @Override
    ...
    public String getDescription() {
        return "Set the time to morning for a world";
    }
    ...
    public boolean performAsPlayer(Player player, String[] strings) {
        ...
        if(strings.length == 0) {
            player.getWorld().setTime(1850);
            LightCore.instance.getMessageSender().sendPlayerMessage(
                    player,
                    LightEssentials.messagePrefix + LightEssentials.instance.getMessages().morning());
        ...
        if(strings.length == 1) {
            ...
            LightCore.instance.getMessageSender().sendPlayerMessage(
                    player,
                    LightEssentials.messagePrefix + LightEssentials.instance.getMessages().morningWorld()
                            .replace("#world#", strings[0]));
            LightSounds.onSuccess(player);
            return false;
        }
